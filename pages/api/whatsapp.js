...
function parsePM(text){ const t=text.replace(/\s+/g,' ').trim(); const r={EntryType:'PM'};
  const tr=t.match(/Training\s+([SCMR])\s*@RPE\s*(\d{1,2})/i); if(tr){r.TrainingType=tr[1].toUpperCase(); r.RPE=parseInt(r[2]);}
...
export default async function handler(req, res) {
  if (req.method !== 'POST') return res.status(200).send('up'); // GET check
